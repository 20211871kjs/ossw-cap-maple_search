...
function closeCharacterInfo() {
    const modal = document.getElementById("character-info");
    if (modal) {
        modal.classList.add("hidden"); // 숨기는 클래스 추가
        setTimeout(() => {
            modal.classList.remove("hidden"); // 필요 시 클래스 제거
        }, 300); // transition 시간(0.3초) 이후에 제거
    }
}
    const headers = {
        "x-nxopen-api-key": "" // 발급받은 nxopen_api_key 입력(여기에요여기)
...
function formatCombatPower(combatPower) {
    ...
    if (millions > 0) {
        formatted += `${millions}만 `;
    }
    formatted += thousands.toString().padStart(4, '0'); // 천 단위는 항상 출력
